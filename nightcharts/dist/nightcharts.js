...
define(['d3'],factory);}else 
if(typeof module === 'object' && module.exports){
// Node. Does not work with strict CommonJS, but
// only CommonJS-like environments that support module.exports,
// like Node.
module.exports = factory(require('./d3.js'));}else 
{
...
useViewBox:false,
width:200,
xAccessor:function xAccessor(d,i){return d[0];},

// xAxis... TODO: move to its own config object
xAxis:void 0,
xAxisDataAccessor:function xAxisDataAccessor(data){return [data];},
...
yAccessor:function yAccessor(d,i){return d[1];},
...
area:false,
areaY0Accessor:function areaY0Accessor(d){return d.min;},
areaY1Accessor:function areaY1Accessor(d){return d.max;},
...
adjustXDomainToTime:true,
barBackground:false,
barClassSetter:void 0,

//has precedence over timeUnit.
barWidth:void 0,
...
//used for calculating a barWidth on time scaled bars, when barWidth is not set.
timeUnit:void 0};
...
dataTransform:function dataTransform(data){return data;},
...
return f1;}else 
if(a != null && a['@@functional/placeholder'] === true){
return f1;}else 
{
return fn.apply(this,arguments);}};};
...
if(n === 0){
return f2;}else 
if(n === 1 && a != null && a['@@functional/placeholder'] === true){
return f2;}else 
if(n === 1){
return _curry1(function(b){
return fn(a,b);});}else 

if(n === 2 && a != null && a['@@functional/placeholder'] === true && b != null && b['@@functional/placeholder'] === true){
return f2;}else 
if(n === 2 && a != null && a['@@functional/placeholder'] === true){
return _curry1(function(a){
return fn(a,b);});}else 

if(n === 2 && b != null && b['@@functional/placeholder'] === true){
return _curry1(function(b){
return fn(a,b);});}else 
...
if(n === 3 && a != null && a['@@functional/placeholder'] === true && b != null && b['@@functional/placeholder'] === true && c != null && c['@@functional/placeholder'] === true){
return f3;}else 
if(n === 3 && a != null && a['@@functional/placeholder'] === true && b != null && b['@@functional/placeholder'] === true){
return _curry2(function(a,b){
return fn(a,b,c);});}else 

if(n === 3 && a != null && a['@@functional/placeholder'] === true && c != null && c['@@functional/placeholder'] === true){
return _curry2(function(a,c){
return fn(a,b,c);});}else 

if(n === 3 && b != null && b['@@functional/placeholder'] === true && c != null && c['@@functional/placeholder'] === true){
return _curry2(function(b,c){
return fn(a,b,c);});}else 

if(n === 3 && a != null && a['@@functional/placeholder'] === true){
return _curry1(function(a){
return fn(a,b,c);});}else 

if(n === 3 && b != null && b['@@functional/placeholder'] === true){
return _curry1(function(b){
return fn(a,b,c);});}else 

if(n === 3 && c != null && c['@@functional/placeholder'] === true){
return _curry1(function(c){
return fn(a,b,c);});}else 
...
var _slice=function _slice(_x,_x2,_x3){var _arguments=arguments;var _again=true;_function: while(_again) {var args=_x,from=_x2,to=_x3;
...
list = 
idx = 
len = undefined;_again = false;switch(_arguments.length){case 1:_arguments = [_x = args,_x2 = 0,_x3 = args.length];_again = true;continue _function;case 2:_arguments = [_x = args,_x2 = from,_x3 = args.length];_again = true;continue _function;default:var list=[];var idx=0;var len=Math.max(0,Math.min(args.length,to) - from);
while(idx < len) {
...
if(not_override){
target_clone[prop] = target_clone[prop]?target_clone[prop]:source[prop];}else 
{
...
if(isObject(o.value)){
state[o.key] = extend(o.value,x);}else 
{
state[o.key] = x;}
...
/**
   ...
   */
var snapToValue=function snapToValue(_x4,_x5){var _again2=true;_function2: while(_again2) {var arr=_x4,value=_x5;
middleValIdx = 
middleVal = undefined;_again2 = false;var middleValIdx=Math.floor(arr.length / 2);var middleVal=arr[middleValIdx];
if(value === middleVal){
return value;}else 
if(arr.length > 2 && value < middleVal){_x4 = 
arr.slice(0,middleValIdx + 1);_x5 = value;_again2 = true;continue _function2;}else 
if(arr.length > 2 && value > middleVal){_x4 = 
arr.slice(middleValIdx);_x5 = value;_again2 = true;continue _function2;}else 
if(arr.length == 2){
...
xDomain = isNestedDataObj(data)?data[0].map(config.xAccessor):data.map(config.xAccessor);}else 
if(!config.xDomain && config.xScaleType != 'ordinal'){
xDomain = [min(data,config.xAccessor),max(data,config.xAccessor)];}else 
{
...
xScale = d3.scale[xScaleType]().domain(domain).rangeRoundBands(range,0.1);}else 
if(config.xScaleType == 'time'){
xScale = d3.time.scale().domain(domain).range(range);}else 
{
...
return isStackedData(data)?function(d){return d.y0 + d.y;}:config.yAccessor;}
...
yDomain = [yMin,yMax];}else 
if(!config.yDomain && yMin > 0){
yDomain = [0,yMax];}else 
{
yDomain = config.yDomain;}

if(!yDomain && config.minYMax || config.minYMax > yDomain[1]){
yDomain[1] = config.minYMax;}

if(config.yMarkers && config.yMarkers.data){
var maxVal=d3.max(config.yMarkers.data);
if(maxVal > yDomain[1]){
yDomain[1] = maxVal;}}
...
yScale = d3.time.scale().domain(domain).range(range);}else 
{
...
function adjustDomainForBarOnTimescale(config,data,xDomain){
var adjustedDomain=[];
if(!config.barWidth && !config.timeUnit){
return adjustedDomain = xDomain;}else 
{
var t1=xDomain[0] instanceof Date?xDomain[0].getTime():xDomain[0];
var t2=xDomain[1] instanceof Date?xDomain[1].getTime():xDomain[1];
var delta=undefined;
if(!config.barWidth && config.timeUnit){
delta = Math.round(config.timeUnit / 2);}else 
if(config.barWidth){
//TODO: this needs some visual testing!
var xRange=getXRange(config,data);
var _xScale=getXScale(config,xDomain,xRange,data);
delta = _xScale.invert(config.barWidth / 2).getTime() - t1;}

adjustedDomain.push(t1 - delta);
adjustedDomain.push(t2 + delta);}
...
var reduceAxisDensity=function reduceAxisDensity(_x6,_x7,_x8){var _again3=true;_function3: while(_again3) {var config=_x6,data=_x7,_dimension=_x8;
dimension = 
txtDensity = undefined;_again3 = false;var dimension=_dimension || 'width';var txtDensity=getTxtDensity(config,data,dimension);
if(txtDensity > config.axisMaxTxtDensity[dimension]){
data = sparseData(data);}

txtDensity = getTxtDensity(config,data,dimension);
if(txtDensity > config.axisMaxTxtDensity[dimension]){_x6 = 
config;_x7 = data;_x8 = dimension;_again3 = true;continue _function3;}else 
{
return data;}}};
...
return labelConfig.textAnchor;}else 
{
return 'start';}}
...
focalPoint = {x:focalPoints.x1,y:focalPoints.y1};}else 
if(labelConfig.macroPosition == 'end'){
focalPoint = {x:focalPoints.x2,y:focalPoints.y2};}else 
/*defaults to end*/{
...
function setMarkers(container,_config,graphData,type,append){
if(!type){
...
yDomain = [0,1];
if(config.yScaleType == 'ordinal'){config.yScaleType = 'linear';}}else 
{
xDomain = [0,1];
...
yScale = utils.getYScale(config,yDomain,yRange,yDomain);}else 
{
...
if(markerG.empty()){
markerG = append?container.append('g'):container.insert('g','g');
markerG.attr('class','nc-' + type + '-markers-grp');}
...
line.
attr('y1',function(d){
var y1=iO?Math.round(xScale(d)):yScale(yDomain[0]);
focalPoints.y1 = y1;
return y1;}).

attr('y2',function(d){
var y2=iO?Math.round(xScale(d)):yScale(yDomain[1]);
focalPoints.y2 = y2;
return y2;}).

attr('x1',function(d){
var x1=iO?yScale(yDomain[0]):Math.round(xScale(d));
focalPoints.x1 = x1;
return x1;}).

attr('x2',function(d){
var x2=iO?yScale(yDomain[1]):Math.round(xScale(d));
focalPoints.x2 = x2;
return x2;});}else 
...
line.
attr('x1',function(d){
var x1=iO?Math.round(yScale(d)):xScale(xDomain[0]);
focalPoints.x1 = x1;
return x1;}).

attr('y1',function(d){
var y1=iO?xScale(xDomain[0]):Math.round(yScale(d));
focalPoints.y1 = y1;
return y1;}).

attr('x2',function(d){
var x2=iO?Math.round(yScale(d)):xScale(xDomain[1]);
focalPoints.x2 = x2;
return x2;}).

attr('y2',function(d){
var y2=iO?xScale(xDomain[1]):Math.round(yScale(d));
focalPoints.y2 = y2;
return y2;});}else 
...
var labelConf=config[markerKey].labels[i];
labelHelpers['set' + type.toUpperCase() + 'MarkerLabel'](g,config,labelConf,focalPoints);}})();}else 
...
return 'translate(' + val + ')';}else 
if(type == 'y' && !iO || type == 'x' && iO){
...
function setXMarkers(container,config,graphData,append){
if(!config.xMarkers.data || config.xMarkers.data.length === 0){return;}

return setMarkers(container,config,graphData,'x',append);}
;

function setYMarkers(container,config,graphData,append){
if(!config.yMarkers.data || config.yMarkers.data.length === 0){return;}

return setMarkers(container,config,graphData,'y',append);}
;
...
var urlBase=isBaseInHead && document.location.href.indexOf('#') == -1?document.location.href:'';
var container=g.
...
var setSymbol=function setSymbol(className,config,data,yScale,xScale,xAccessor,symbol,_duration,idx,dispatch){
...
return "translate(" + xScale(x) + "," + yScale(y) + ")";}else 
{
return null;}}).
...
cssClass = config.symbolClassSetter.call(this,d);}else 
{
cssClass = '';}
...
if(!dispatch){return;}
dispatch.customClick(d,i);
d3.event.stopPropagation();}).

on("mouseover",function(d,i){
if(!dispatch){return;}
d3.select(this).classed('active',true);
dispatch.customMouseOver(d,i);
d3.event.stopPropagation();}).

on("mouseout",function(d,i){
if(!dispatch){return;}
d3.select(this).classed('active',false);
dispatch.customMouseOut(d,i);
d3.event.stopPropagation();});
...
function _getBarWidthWithTimeScale(config,data,_spaceInBetween){
if(!config.barWidth && !config.timeUnit){
return 1;}else 
{
...
var originalChartWidth=chartW - config.xOffset;
var timeSpan=utils.getTimespan(xDomain[0],xDomain[1]);
var timeInstances=Math.round(timeSpan / config.timeUnit);
return Math.round((originalChartWidth - (timeInstances + 1) * spaceInBetween) / (timeInstances + 1));}}



function getBarWidth(config,data,xScale,spaceInBetween){
var barW;
if(config.barWidth){
barW = config.barWidth;}else 
{
if(config.xScaleType == 'ordinal'){
barW = xScale.rangeBand();}else 
if(config.xScaleType == 'time'){
barW = _getBarWidthWithTimeScale(config,data,spaceInBetween);}else 
{
...
var barRange;
if(config.xScaleType == 'ordinal'){
barRange = xScale.range();}else 
if(config.xScaleType == 'time'){
var spaceInBetween=_spaceInBetween || 0;
if(data.length == 1){
//TMP, FIX xScale(config.xAccessor(d)))
barRange = [Math.round(spaceInBetween / 2)];}else 
{
barRange = data.map(function(d){
return Math.floor(xScale(config.xAccessor(d))) - Math.round(barW / 2);});}}else 
...
preserveAspectRatio:'xMinYMin meet'});}else 
...
function getOutExtentParamsLeft(config,brushExtent){
var chartH=utils.getChartHeight(config);
var outExtentParams={};

outExtentParams.x = 0 - config.xOffset;
outExtentParams.y = 0;
outExtentParams.width = brushExtent[0] + config.xOffset;
outExtentParams.height = chartH;

return outExtentParams;}


function getOutExtentParamsRight(config,brushExtent,svgBasics){
var chartH=utils.getChartHeight(config);
var chartW=utils.getChartWidth(config);
var outExtentParams={};

outExtentParams.x = brushExtent[1];
outExtentParams.y = 0;
outExtentParams.width = chartW - brushExtent[1] + config.xOffset;
outExtentParams.height = chartH;

return outExtentParams;}


// getOutExtentParams :: {} -> [] -> String -> {}
function getOutExtentParams(config,side,brushExtent){
if(brushExtent[0] == brushExtent[1]){
return {x:0,y:0,width:0,height:0};}

if(side == 'left'){
return getOutExtentParamsLeft(config,brushExtent);}else 
if(side == 'right'){
return getOutExtentParamsRight(config,brushExtent);}else 
{
throw new Error('Wrong getOutExtentParams side argument');}}
...
callbacks.snapend.call(this,brush);}});}else 
...
return {
getOutExtentParams:getOutExtentParams,
...
gBrush = container.
selectAll(".brush")
//FIXME, avoid empty data objects, use alternative if no meaningful data is available
.data([[]]);
gBrush.enter().
append("g").
attr("class","brush").
...
if(config.brush.brushMasks){

var leftMask=gBrush.select('.nc-brush-left-mask');
if(leftMask.empty()){
gBrush.
insert('rect','rect').
classed('nc-brush-mask',true).
classed('nc-brush-left-mask',true);}


var rightMask=gBrush.select('.nc-brush-right-mask');
if(rightMask.empty()){
gBrush.
insert('rect','rect').
classed('nc-brush-mask',true).
classed('nc-brush-right-mask',true);}}
...
if(config.brush.brushMasks){
var gBrush=d3.select(this);
//FIXME: there is a bug here! Not sure what is going on!!!
var leftMaskRect=gBrush.select('.nc-brush-left-mask');
var rightMaskRect=gBrush.select('.nc-brush-right-mask');
var extent=brush.extent();
var outExtent=[xScale(extent[0]),xScale(extent[1])];
var leftMaskParams=helpers.getOutExtentParams(config,'left',outExtent);
leftMaskRect.
attr('x',leftMaskParams.x).
attr('y',leftMaskParams.y).
attr('width',leftMaskParams.width).
attr('height',leftMaskParams.height);
var rightMaskParams=helpers.getOutExtentParams(config,'right',outExtent);
rightMaskRect.
attr('x',rightMaskParams.x).
attr('y',rightMaskParams.y).
attr('width',rightMaskParams.width).
attr('height',rightMaskParams.height);}}



var svgBasics=utils.getSvgBasics(config,_data);var _svgBasics=_slicedToArray(
svgBasics,9);var chartW=_svgBasics[0];var chartH=_svgBasics[1];var invertOrientation=_svgBasics[2];var xRange=_svgBasics[3];var yRange=_svgBasics[4];var xDomain=_svgBasics[5];var yDomain=_svgBasics[6];var xScale=_svgBasics[7];var yScale=_svgBasics[8];
...
brushOptions.callbacks.brush = onBrushSlider;
...
if(brushExtent){
extent = brushExtent;}else 
if(config.brush.extent){
extent = config.brush.extent === 'full'?[xMin,xMax]:config.brush.extent;}else 
{
null;}
...
if(!exports.properties.brush){exports.properties.brush = brush;}
...
nightcharts.tipComponentHelpers = function(){
'use strict';

var utils=nightcharts.utils();
var barHelpers=nightcharts.barComponentHelpers();

function getTipSelector(config,d){
if(d.values){
return d.values.reduce(function(prev,curr,i){
var comma=i > 0?',':'';
return prev + (comma + ' .tip-' + curr.idx);},
'');}else 
if(config.tip && config.tip.keyAccessor){
//TODO: this is a bit of a hack. FIXME.
var dd=d[config.tip.keyAccessor]?d[config.tip.keyAccessor]:d;
return '.tip-' + dd.idx;}else 
{
return '.tip-' + d.idx;}}



function getBarParamsDataUnit(config,data,i){
var datapoint=data[i];
var d;
if(datapoint.key){
d = datapoint.key;}else 
{
d = config.xAccessor(datapoint);}

if(config.xScaleType == 'time'){
return isNaN(+d)?+new Date(d):+d;}else 
if(config.xScaleType == 'ordinal'){
return d;}}



function getBarParamsTime(config,data,xScale,i){
var d0;
var d01;
var d1;
var d12;
...
d1 = getBarParamsDataUnit(config,data,i);
d2 = getBarParamsDataUnit(config,data,i + 1);
d12 = d1 + Math.floor((d2 - d1) / 2);
x0 = Math.floor(xScale(d1));
x1 = Math.ceil(xScale(d12));}else 
if(data[i + 1] === undefined){
d0 = getBarParamsDataUnit(config,data,i - 1);
d1 = getBarParamsDataUnit(config,data,i);
d01 = d1 - Math.floor((d1 - d0) / 2);
x0 = Math.floor(xScale(d01));
x1 = Math.ceil(xScale(d1));}else 
{
d0 = getBarParamsDataUnit(config,data,i - 1);
d1 = getBarParamsDataUnit(config,data,i);
d2 = getBarParamsDataUnit(config,data,i + 1);
d01 = d1 - Math.floor((d1 - d0) / 2);
d12 = d2 - Math.floor((d2 - d1) / 2);
x0 = Math.floor(xScale(d01));
x1 = Math.ceil(xScale(d12));}


return {width:x1 - x0,x0:x0};}


//TODO: Memoization
function getBarParams(config,data,xScale,i){
if(config.xScaleType == 'time'){
return getBarParamsTime(config,data,xScale,i);}else 
if(config.xScaleType == 'ordinal'){
var _barW=barHelpers.getBarWidth(config,data,xScale);
var _barRange=barHelpers.getBarRange(config,data,xScale,_barW);
return {width:_barW,x0:_barRange[i]};}}



return {
getBarParams:getBarParams,
getTipSelector:getTipSelector};};
...
var utils=nightcharts.utils();
var barHelpers=nightcharts.barComponentHelpers();
var tipHelpers=nightcharts.tipComponentHelpers();
var config=utils.setConfig(_config);
var curry=utils.curry;
...
var args;
var selector=tipHelpers.getTipSelector(config,d);
var element=selection.selectAll(selector);

element.classed('active',true);

if(d.values){
args = d.values.map(function(d,i){
return {
x:d.x,
y:d.y,
label:selection.datum()[i].label};});}else 


if(config.tip.keyAccessor){
//TODO: this is a bit of a hack. FIXME.
args = [d[config.tip.keyAccessor]?d[config.tip.keyAccessor]:d];}else 
{
args = [{
x:config.xAccessor(d),
y:config.yAccessor(d),
label:d.label}];}
...
// Mostly for angular related apps: TODO: move to method, also used in setChartGroupsContainer.
var isBaseInHead=d3.select('base')[0][0];
var urlBase=isBaseInHead?document.location.href:'';
var clipId=config.listenToBrushEvents?_selection.datum().clipId:void 0;

// Flatten the data and set an idx property on each data point.
var _data=utils.flattenArray(_selection.datum());
var svgBasics=utils.getSvgBasics(config,_data);var _svgBasics2=_slicedToArray(
svgBasics,9);var chartW=_svgBasics2[0];var chartH=_svgBasics2[1];var invertOrientation=_svgBasics2[2];var xRange=_svgBasics2[3];var yRange=_svgBasics2[4];var xDomain=_svgBasics2[5];var yDomain=_svgBasics2[6];var xScale=_svgBasics2[7];var yScale=_svgBasics2[8];
...
if(config.tip && config.tip.rollover == 'native'){
//Native. Do not use voronoi or rect artifacts to intercept the mouse, but
//rely on the svg graph itself.
_selection.selectAll('.nc-chart-group rect').each(function(d){
d3.select(this).
on('mouseover',partAppliedShowTip).
on('mouseout',partAppliedRemoveTip);});}else 

if(config.tip && config.tip.rollover == 'voronoi'){(function(){
...
attr('d',setVoronoiPath).
datum(setVoronoiDatum).
on('mouseover',partAppliedShowTip);}});})();}else 



if(config.tip && config.tip.rollover == 'rect'){(function(){
...
var tipData=d3.nest().key(config.xAccessor).entries(_data);
var invertOrientation=config.invertOrientation;
var chartH=utils.getChartHeight(config);
var xDomain=utils.getXDomain(config,_data);
...
var adjustedXDomain=utils.adjustDomainForBarOnTimescale(config,_data,xDomain);
var adjustedXScale=utils.getXScale(config,adjustedXDomain,xRange,_data);
partAppliedShowTip = curriedShowTip(_selection,config,yScale,adjustedXScale);}else 
{
partAppliedShowTip = curriedShowTip(_selection,config,yScale,xScale);}


var getBarParamsCurr=curry(tipHelpers.getBarParams);
var getBarParamsCurrPAppl=getBarParamsCurr(config,tipData,xScale);


var setRectAttr=function setRectAttr(invertOrientation,chartH,barParams){
return {
x:function x(d,i){
if(invertOrientation){
return 0;}

return barParams(i).x0; //barRange[i];
},
y:function y(d,i){
if(invertOrientation){
return barParams(i).x0;}
...
rects.enter().
append("rect").
attr(setRectAttr(invertOrientation,chartH,getBarParamsCurrPAppl));
rects.exit().remove();
...
var _xScale2=utils.getXScale(config,_xDomain2,xRange,_data);

if(config.graphType == 'bar' && config.xScaleType == 'time'){
var adjustedXDomain=utils.adjustDomainForBarOnTimescale(config,_data,_xDomain2);
var adjustedXScale=utils.getXScale(config,adjustedXDomain,xRange,_data);
partAppliedShowTip = curriedShowTip(_selection,config,yScale,adjustedXScale);}else 
{
partAppliedShowTip = curriedShowTip(_selection,config,yScale,_xScale2);}


var _getBarParamsCurr=curry(tipHelpers.getBarParams);
var _getBarParamsCurrPAppl=_getBarParamsCurr(config,tipData,_xScale2);

rects.attr(setRectAttr(invertOrientation,chartH,_getBarParamsCurrPAppl)).
on('mouseover',partAppliedShowTip);}});})();}}
...
if(invertOrientation){
if(isStacked){
return yScale(yAccessor(d) - d.y);}else 
{
...
if(config.xScaleType == 'time' && config.adjustXDomainToTime){
var adjustedXDomain=utils.adjustDomainForBarOnTimescale(config,_data,xDomain);
...
var spaceInBetween=Math.max(Math.round(chartW * 0.2 / utils.getLongerArr(_data).length),1);
var barW=barHelpers.getBarWidth(config,_data,xScale,spaceInBetween);
// _data[0] FIXME, this is only TMP, need to consider all data!
var barRange=barHelpers.getBarRange(config,_data[0],xScale,barW,spaceInBetween);
...
if(yKey){
d[yKey] = yDomain[1];}else 
{
d[1] = yDomain[1];}

return d;});

var backs=_selection.select('.nc-chart-group').
selectAll('.bar-back').
data(backData);
...
var xMarkers=helpers.setXMarkers(container,config,_data,true);
var yMarkers=helpers.setYMarkers(container,config,_data,true);}
...
var dataWidth;
if(config.roundBins){
dataWidth = Math.round((Math.abs(config.xDomain[0]) + Math.abs(config.xDomain[1])) / n,0);}else 
{
...
var dispatch=d3.dispatch('customHover','customClick','customMouseOver','customMouseOut','filter','nativeEvent');
...
bind(this,'dots',config,_d,yScale,xScale,config.xAccessor,symbol,duration,i,dispatch);
setSymbol();}
...
var svgBasics=utils.getSvgBasics(config,_data);var _svgBasics3=_slicedToArray(
svgBasics,9);var chartW=_svgBasics3[0];var chartH=_svgBasics3[1];var invertOrientation=_svgBasics3[2];var xRange=_svgBasics3[3];var yRange=_svgBasics3[4];var xDomain=_svgBasics3[5];var yDomain=_svgBasics3[6];var xScale=_svgBasics3[7];var yScale=_svgBasics3[8];
...
return d[config.classSetterOnData];}else 
{
return i;}});
...
lines.each(function(d,i){
updateLines.call(this,d,i,0);});}else 
...
if(config.listenToBrushEvents && id !== config.id){
xScale.domain(brush.empty()?masterXScale.domain():brush.extent());
_selection.select('.x-axis-group.axis').call(xAxis);}else 
{
//This is the native event, the one fired from the user interacting with the app!
//dispatch.nativeEvent(id);
}});}
...
if(exports.config.xScaleType == 'time'){
var xDomain=utils.getXDomain(config,data);
var adjustedXDomain=utils.adjustDomainForBarOnTimescale(config,data,xDomain);
xAxisComponent.xDomain(adjustedXDomain);}
...
_selection.each(function(_data){
if(!utils.checkForData(config,_data)){return;}
var svgBasics=utils.getSvgBasics(config,_data);var _svgBasics4=_slicedToArray(
svgBasics,9);var chartW=_svgBasics4[0];var chartH=_svgBasics4[1];var invertOrientation=_svgBasics4[2];var xRange=_svgBasics4[3];var yRange=_svgBasics4[4];var xDomain=_svgBasics4[5];var yDomain=_svgBasics4[6];var xScale=_svgBasics4[7];var yScale=_svgBasics4[8];
...
tipComponent.tip({rollover:'voronoi'});}else 
if(!config.tip.rollover && dataLength == 1){
tipComponent.tip({rollover:'rect'});}
...
//# sourceMappingURL=nightcharts.js.map