...
nightcharts.utils = function() {
  ...
  function getYDomain(config, data) {
    ...
    if (config.yMarkers && config.yMarkers.data) {
      let maxVal = d3.max(config.yMarkers.data);
      if (maxVal > yDomain[1]) {
        yDomain[1] = maxVal;
      }
    }
  ...
  function adjustDomainForBarOnTimescale (config, data, xDomain) {
    ...
    if (!config.barWidth && !config.timeUnit) {
      return adjustedDomain = xDomain;
    } else {
      let t1 = xDomain[0] instanceof Date ? xDomain[0].getTime() : xDomain[0];
      let t2 = xDomain[1] instanceof Date ? xDomain[1].getTime() : xDomain[1];
      let delta;
      if (!config.barWidth && config.timeUnit) {
        delta = Math.round(config.timeUnit/2);
      }  else if (config.barWidth) {
        //TODO: this needs some visual testing!
        let xRange = getXRange(config, data);
        let xScale = getXScale(config, xDomain, xRange, data);
        delta = xScale.invert(config.barWidth/2).getTime() - t1;
      }
...
nightcharts.domHelpers = function() {
  ...
  var setSymbol = function(className, config, data, yScale, xScale, xAccessor, symbol, _duration, idx, dispatch) {
...
nightcharts.barComponentHelpers = function() {
  ...
  function _getBarWidthWithTimeScale (config, data, _spaceInBetween) {
    if (!config.barWidth && !config.timeUnit) {
      ...
      let timeInstances = Math.round(timeSpan / config.timeUnit);
  ...
  function getBarWidth (config, data, xScale, spaceInBetween) {
    ...
    if (config.barWidth) {
      ...
      if (config.xScaleType == 'ordinal') {
        ...
        barW = _getBarWidthWithTimeScale(config, data, spaceInBetween);
      } else {
...
nightcharts.brushComponentHelpers = function() {
  ...
  function getOutExtentParams (config, side, brushExtent) {
    ...
    if (side == 'left') {
      return getOutExtentParamsLeft(config, brushExtent);
    } else if (side == 'right') {
      return getOutExtentParamsRight(config, brushExtent);
    } else {
    ...
    function setBrush(container, data, config, brush) {
      ...
      if (brush) {
        ...
        if (config.brush.brushMasks) {
          ...
          let leftMask = gBrush.select('.nc-brush-left-mask');
          if (leftMask.empty()) {
            gBrush
              .insert('rect', 'rect')
          ...
          let rightMask = gBrush.select('.nc-brush-right-mask');
          if (rightMask.empty()) {
            gBrush
              .insert('rect', 'rect')
                .classed('nc-brush-mask', true)
...
nightcharts.tipComponentHelpers = function() {
  ...
  function getTipSelector (config, d) {
    ...
    } else if (config.tip && config.tip.keyAccessor) {
      //TODO: this is a bit of a hack. FIXME.
      let dd = d[config.tip.keyAccessor] ? d[config.tip.keyAccessor] : d;
      return `.tip-${dd.idx}`;
    } else {
      return `.tip-${d.idx}`;
    }
  ...
  function getBarParams (config, data, xScale, i) {
    if (config.xScaleType == 'time') {
      return getBarParamsTime(config, data, xScale, i);
    } else if (config.xScaleType == 'ordinal') {
      let barW = barHelpers.getBarWidth(config, data, xScale);
      let barRange = barHelpers.getBarRange(config, data, xScale, barW);
      return {width: barW, x0: barRange[i]};
    }
  }
    ...
    if (d.values) {
      args = d.values.map(function(d, i) {
        return {
          x: d.x,
          y: d.y,
          label: selection.datum()[i].label,
        };
      });
    } else if (config.tip.keyAccessor) {
      //TODO: this is a bit of a hack. FIXME.
      args = [d[config.tip.keyAccessor] ? d[config.tip.keyAccessor] : d];
    } else {
      args = [{
...
//    }
  ...
  function exports(_selection) {
    ...
    if (config.tip && config.tip.rollover == 'native') {
    ...
    } else if (config.tip && config.tip.rollover == 'voronoi') {
      ...
      let tipData = d3.nest().key(config.xAccessor).entries(_data);
      ...
      if (config.graphType == 'bar' && config.xScaleType == 'time') {
        let adjustedXDomain = utils.adjustDomainForBarOnTimescale(config, _data, xDomain);
      ...
      let getBarParamsCurr = curry(tipHelpers.getBarParams);
      let getBarParamsCurrPAppl = getBarParamsCurr(config, tipData, xScale);


      let setRectAttr = function(invertOrientation, chartH, barParams) {
        return {
          x: function(d, i) {
      ...
      brushComponentDispatch.on(`brush.${config.id}tip`, function(brush, masterXScale, id) {
        if (config.listenToBrushEvents && id !== config.id) {
          ...
          rects.attr(setRectAttr(invertOrientation, chartH, getBarParamsCurrPAppl))
            .on('mouseover', partAppliedShowTip);
        }
      });
...
nightcharts.bar = function() {
  ...
  function exports(_selection) {
    ...
    _selection.each(function(_data) {
      ...
      if (exports.config.xScaleType == 'time') {
        let xDomain = utils.getXDomain(config, data);
        let adjustedXDomain = utils.adjustDomainForBarOnTimescale(config, data, xDomain);
        xAxisComponent.xDomain(adjustedXDomain);
      }
